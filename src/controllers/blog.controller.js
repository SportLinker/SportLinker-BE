...
class BlogController {
    ...
    async createNewBlog(req, res, next) {
        new CREATED({
            ...
            metadata: await BlogService.createNewBlog(req.body, req.user),
        }).send(res)
    }
    ...
    async getReactList(req, res, next) {
        new Ok({
            message: 'React list',
    ...
    async getBlogDetail(req, res, next) {
        new Ok({
            message: 'Blog detail',
            metadata: await BlogService.getBlogDetail(req.params.blog_id, req.user.id),
        }).send(res)
    }
}
